...
import { initializeUserBalance, subscribeToUserData } from 'js/core/balance-manager.js';
...
async function loadTopPlayers() {
    try {
        const { collection, getDocs, query, orderBy, limit } = await import('js/core/firebase-config.js');
        const { db } = await import('js/core/firebase-config.js');
...
function setupEventListeners() {
    ...
    document.getElementById('heroCTABtn').addEventListener('click', () => {
        window.location.href = 'pages/games/dice.html';
    });

    // Hero VIP button
    document.getElementById('heroVIPBtn').addEventListener('click', () => {
        window.location.href = 'pages/games/vip.html';
    });
...
async function loadGlobalStats() {
    try {
        const { collection, getDocs } = await import('js/core/firebase-config.js');
        const { db } = await import('js/core/firebase-config.js');
